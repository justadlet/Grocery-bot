...
def get_product_list(user_id):
    ...
    for i in products:
        ...
        if i[0][0] == 'v':
            x = int(encrypted[1:]) - 1
            print(x)
            decrypted_product = menu.vegetables[x][0] + " x " + str(i[1])
            whole_price += int(menu.vegetables[x][1]) * int(i[1])
        elif i[0][0] == 'f':
            x = int(encrypted[1:]) - 1
            print(x)
            decrypted_product = menu.fruits[x][0] + " x " + str(i[1])
            whole_price += int(menu.fruits[x][1]) * int(i[1])
        elif i[0][0] == 'm':
            x = int(encrypted[1:]) - 1
            print(x)
            decrypted_product = menu.meals[x][0] + " x " + str(i[1])
            whole_price += int(menu.meals[x][1]) * int(i[1])
        elif i[0][0] == 'd':
            x = int(encrypted[1:]) - 1
            print(x)
            decrypted_product = menu.derinks[x][0] + " x " + str(i[1])
